...
.. warning::
    ...
    when it comes to the node.js test server or the ``ImpreseeGuzzleHttp\Tests\Server``
    class. If you are using the test server or ``Server`` class outside of
    ImpreseeGuzzleHttp/guzzle, then you will need to configure autoloading and
...
``ImpreseeGuzzleHttp\Tests\Server`` provides a static interface to the test server. You

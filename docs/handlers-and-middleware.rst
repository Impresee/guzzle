...
``ImpreseeGuzzleHttp\HandlerStack::create(callable $handler = null)`` static method.
...
When provided no ``$handler`` argument, ``ImpreseeGuzzleHttp\HandlerStack::create()``
...
``ImpreseeGuzzleHttp\Middleware::mapRequest()`` middleware. This middleware accepts
...
``ImpreseeGuzzleHttp\Middleware::mapResponse()`` middleware.
